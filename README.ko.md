...
네이버 부동산 스크래퍼. anti-bot 우회 + 갭투자 매물 자동 필터링. 한국 부동산 시장 조사 및 투자 기회 발굴용.
...
- 마우스 이동 시뮬레이션: 20단계로 부드럽게 이동, 봇 티 안 남
- 그리드 스윕 알고리즘: 체계적이지만 패턴이 명확하지 않은 수집 방식
- 리소스 차단 최적화: 이미지/폰트 차단으로 빠른 로딩 + 실제 브라우저처럼 동작

최근 네이버 업데이트로 대부분의 스크래퍼가 차단되어, 교육 목적으로 직접 만든 네이버 부동산 스크래퍼.
...
한국 영토 범위 정의. 스크래퍼가 이 범위를 벗어나지 않도록 제한.
- 제주도 포함: 33.0 (최남단)
- 강원도 북부: 39.5 (최북단)
- 서해안: 124.0 (최서단)
- 동해안: 132.1 (최동단)
...
`True`일 때 기전세금 ≥ 매매가인 매물만 최종 결과에 포함.
- 갭투자 후보만 선별
...
단지별 최소 매물 개수. 이 값 미만인 단지는 우선순위가 낮아짐.
...
주의: 너무 많으면 IP 차단 위험 증가

```python
BLOCK_HEAVY_RESOURCES = True
```

이미지, 폰트, 미디어 등 무거운 리소스 차단.
...
실행 완료 시 `매물정보_확장_YYYYMMDD_HHMMSS.xlsx` 파일 생성.
...
전략: 전체 그리드를 순서대로 방문하지 않고, 위/아래 행만 스캔해서 패턴 숨김.
...
위도/경도를 픽셀 좌표로 변환해서 정확한 맵 드래그 제어.
...
이 코드는 교육 및 연구 목적의 참고 구현.

## 주의사항 및 법적 고지

### 법적 준수사항

이 도구는 교육 및 연구 목적으로 제작됨.
...
- 과도한 요청으로 서버에 부담 주지 않을 것
- 적절한 딜레이 설정 (현재 코드는 안전한 수준)
...
- 네이버는 언제든지 플랫폼 구조 변경 가능
- IP 차단 위험 있으니 적절한 간격으로 실행할 것
- 공용 IP에서 여러 명이 동시 실행 시 차단 위험

### 업데이트 정책

- 이 코드는 2025년 10월 기준
- 네이버 플랫폼 변경 시 업데이트 필요할 수 있음
- Star 누르면 업데이트 소식 받을 수 있음

---

### Star & Update

이 프로젝트가 도움이 됐다면 Star 부탁.
...
자유롭게 사용, 수정, 배포 가능. 단, 원저작자 표시 유지할 것.
